...
@IonicPage()
@Component({
  selector: 'page-company-edit-job',
  templateUrl: 'company-edit-job.html',
})
export class CompanyEditJobPage {

  patient: any;
  constructor(public navCtrl: NavController, public navParams: NavParams, private auth: AuthService, private db:AngularFireDatabase) {
  ...
  schedule(f) {
    ...
    data['patient'] = this.patient.patient;
    data['Dname'] = this.patient.Dname;
    data['email'] = this.patient.email;
    data['hospital'] = this.auth.currentUser.uid;
    console.log(data);
    this.db.database.ref('schedule/'+this.patient.key).set(data);
    this.navCtrl.pop();
  }
  ionViewDidLoad() {
    console.log('ionViewDidLoad CompanyEditJobPage');
  }
